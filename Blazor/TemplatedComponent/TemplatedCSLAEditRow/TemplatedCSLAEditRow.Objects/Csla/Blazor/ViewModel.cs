...
namespace TemplatedCSLAEditRow.Objects.Blazor
{
    /// <summary>
    ...
    public class ViewModel<T>
    {
        ...
        {
            if (typeof(Csla.Core.IReadOnlyObject).IsAssignableFrom(typeof(T)) ||
                typeof(Csla.Core.IReadOnlyCollection).IsAssignableFrom(typeof(T)) ||
                typeof(Csla.Core.IEditableCollection).IsAssignableFrom(typeof(T)))
            {
                if (Csla.Server.DataPortal.GetCriteriaFromArray(parameters) is Csla.Server.EmptyCriteria)
        ...
        {
            if (Model is Csla.Core.ITrackStatus obj && !obj.IsSavable)
            {
        ...
        protected virtual async Task<T> DoSaveAsync()
        {
            if (Model is Csla.Core.ISavable savable)
            {
                ...
                if (Model is Csla.Core.IEditableBusinessObject editable)
                    new Csla.Core.GraphMerger().MergeGraph(editable, (Csla.Core.IEditableBusinessObject)result);
                else
                    Model = result;
            }
            return Model;
